...
int main(void) {
	...
	uint8_t baud_Rate = 1;					// Sets the Baud rate
	uint8_t pin_Setting = 1;				// selects the pins used
//	uint8_t device_CS = 11;					// selects the SYNC/SS pin (DAC)
	uint8_t device_CS = 5;					// selects the SYNC/SS pin (5k POT)

	init_SPI (baud_Rate, pin_Setting);		// clk_Rate -> 8 = 8MHz, 4 = 4MHz, 2 = 2.67MHz, 1 = 1MHz
...
//    	++dac_Val;
    	...
    	write_uint8_SPI (tx_Data_8, device_CS); 	// device 5 is Hub
    	++tx_Data_8;													// (5k POT)
 //   	flash_LED_1(1,1);
