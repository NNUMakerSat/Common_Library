...
void flash_LED_1(uint8_t clk_Delay1, uint32_t duration1) {		// Flash LED however long you want at whatever frequency you want
	uint8_t i;
	for (g_timer1 = 0; g_timer1 < duration1; ++g_timer1) {
		if (!(P1OUT == BIT0)) {									// If LED 2 is off
			for (i = 0; i < 2; ++i) {
				P1OUT ^= BIT0;
				switch (clk_Delay1){
				case 0:
...
void flash_LED_2(uint8_t clk_Delay2, uint32_t duration2) {	// Flash LED however long you want at whatever frequency you want
	uint8_t i;
	for (g_timer2 = 0; g_timer2 < duration2; ++g_timer2) {
		if (!(P9OUT == BIT7)) {									// If LED 2 is off
			for (i = 0; i < 2; ++i) {
				P9OUT ^= BIT7;
				switch (clk_Delay2){
				case 0:
